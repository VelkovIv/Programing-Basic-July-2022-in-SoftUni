...
vineyeard_area_m2 = int(input())
grape_from_m2 = float(input())
needed_vine_lt = int(input())
worker_numbers = int(input())
vine_production = vineyeard_area_m2 * 0.4
grape_for_vine = vine_production * grape_from_m2
produced_vine = grape_for_vine / 2.5

if produced_vine >= needed_vine_lt :
    produced_vine = math.ceil(produced_vine)
    remain_vine = math.ceil(produced_vine - needed_vine_lt)
    vine_per_person = math.ceil(remain_vine / worker_numbers)
    print(f"Good harvest this year! Total wine: {produced_vine} liters.")
    print(f"{remain_vine} liters left -> {vine_per_person} liters per person.")
else:
    vine_shortage = math.floor(needed_vine_lt - produced_vine)
    print(f"It will be a tough winter! More {vine_shortage} liters wine needed.")